...
export let metaBalls = [];
for (var j = 0; j<mapin[0].length;j++){
  if(j % 3 == 0){
    var xs = mapin[0][j]
    } else if (j % 3 == 1){
      var ys = mapin[0][j]
      ...
      var zs = mapin[0][j]
      console.log(zs)
      metaBalls.push({ center: new THREE.Vector3(xs, ys, zs), radius: 0.001 });
    }
}

//metaBalls.push({ center: new THREE.Vector3(10, 1, 0), radius: 0.2 });
//metaBalls.push({ center: new THREE.Vector3(0, 0, 0), radius: 0.5 });
///metaBalls.push({ center: new THREE.Vector3(0, 0, -2), radius: 0.25 });
